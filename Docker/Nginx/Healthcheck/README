...
Limpa os arquivos de cache do apt para otimizar o tamanho final da imagem.

## Exposição de Porta:

Abre a porta 80 para permitir o tráfego HTTP para o servidor Nginx.

## Node Exporter (Download Remoto):

Utiliza o comando ADD para baixar e descompactar o arquivo node_exporter-1.6.0.linux-amd64.tar.gz diretamente do GitHub para o diretório /root/node-exporter dentro do contêiner. O Node Exporter é uma ferramenta popular para expor métricas de hardware e SO para sistemas de monitoramento como o Prometheus.
...
Em resumo, a imagem Docker resultante será um servidor Nginx pronto para servir a página index.html, com o Node Exporter baixado e configurado para monitoramento, e um mecanismo de verificação de saúde para garantir sua disponibilidade.